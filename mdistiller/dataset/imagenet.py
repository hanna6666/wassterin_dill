...
class ImageNet(ImageFolder):
    def __init__(self, root, transform=None, target_transform=None):
        ...
        # self.samples = self.samples[:500] # for DEBUG
...
def get_imagenet_train_transform(mean, std, input_size=224):
    print('input_size: ', input_size)
    # print('Using jitter: ', 0.4)
...
def get_imagenet_val_loader(val_batch_size, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    ...
    test_loader = torch.utils.data.DataLoader(test_set,
        batch_size=val_batch_size, shuffle=False, num_workers=8, pin_memory=False)
    return test_loader
